 ...
 */
bool
SageTools::collectReadWriteRefs(SgNode* node,
				std::vector<SgNode*>& readRefs,
				std::vector<SgNode*>& writeRefs,
				bool allowMathFunc)
{
 ...
 */
bool
SageTools::collectReadWriteRefsFastImpl(SgNode* node,
					std::vector<SgNode*>& readRefs,
					std::vector<SgNode*>& writeRefs,
					bool allowMathFuncs)
{
	  ...
	  isSgDotExp(node))
	{
	  ...
	  while (isSgCastExp(node))
	    node = node->get_parent();
          ...
          // Special case 3: if it's *a, don't also mark a
          //if( isSgVarRef(node) ){
	  //  while( isSgPointerDerefExp(node) )
