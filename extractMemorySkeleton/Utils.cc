...
/* Query expression statements for function call expressions. If any are found
 * then the statement constains a function call.
 *
 * TODO: Is it possible to have a compound statement that is not an expression
 * statement but still contains a function call?
 *
 */

bool
containsFunctionCall (SgStatement* stmt) {
  if ( stmt ) {
    SgExprStatement* s_expr = isSgExprStatement(stmt);
    if ( s_expr) {
      SgExpression* expr = s_expr->get_expression();    ROSE_ASSERT( expr );
